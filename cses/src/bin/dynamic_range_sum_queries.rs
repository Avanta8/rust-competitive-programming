    ...
    T: Copy,
{
    ...
    pub fn fill(&mut self, items: &[T]) {
        ...
        self.vec
            .splice(idx..idx + items.len(), items.to_vec().into_iter().map(Some));
...
pub fn main() {
    ...
    let mut st = SegmentTree::new(n, |a, b| a + b);
    st.fill(&v);
    ...
    for _ in 0..q {
        ...
        match q {
            1 => {
                st.update(sc.usize0(), sc.read());
            }
            2 => {
                let ans = st.get(sc.usize0()..sc.usize()).unwrap();
                sc.writeln(ans);
            }
            _ => unreachable!(),
        }
